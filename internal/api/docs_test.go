...
func TestDocs_DisabledByDefault(t *testing.T) {
	...
	require.Equal(t, http.StatusNotFound, getPath(srv, "/docs").Code)
	require.Equal(t, http.StatusNotFound, getPath(srv, "/docs/openapi.yaml").Code)
}
...
func TestDocs_EnabledServesSpecAndUI(t *testing.T) {
	...
	spec := getPath(srv, "/docs/openapi.yaml")
	...
	require.Contains(t, ui.Body.String(), "/docs/openapi.yaml")
}
