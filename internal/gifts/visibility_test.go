...
func TestCheckVisibility(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "no gates set is visible",
			gift: domain.Gift{},
			want: Visible,
		},
		{
			name: "scheduled in the future is not yet open",
			gift: domain.Gift{ScheduledOpenAt: ptr(future)},
			want: NotYetOpen,
		},
		{
			name: "scheduled in the past is visible",
			gift: domain.Gift{ScheduledOpenAt: ptr(past)},
			want: Visible,
		},
		{
			name: "scheduled exactly now is visible (inclusive)",
			gift: domain.Gift{ScheduledOpenAt: ptr(now)},
			want: Visible,
		},
		{
			name: "expires in the future is visible",
			gift: domain.Gift{ExpiresAt: ptr(future)},
			want: Visible,
		},
		{
			name: "expires in the past is expired",
			gift: domain.Gift{ExpiresAt: ptr(past)},
			want: Expired,
		},
		{
			name: "expires exactly now is expired (exclusive)",
			gift: domain.Gift{ExpiresAt: ptr(now)},
			want: Expired,
		},
		{
			name: "single open and already opened is consumed",
			gift: domain.Gift{SingleOpen: true, OpenedAt: ptr(past)},
			want: AlreadyOpened,
		},
		{
			name: "single open but never opened is visible",
			gift: domain.Gift{SingleOpen: true},
			want: Visible,
		},
		{
			name: "opened but not single open is visible",
			gift: domain.Gift{SingleOpen: false, OpenedAt: ptr(past)},
			want: Visible,
		},
		{
			name: "all gates present but satisfied is visible",
			gift: domain.Gift{ScheduledOpenAt: ptr(past), ExpiresAt: ptr(future), SingleOpen: true},
			want: Visible,
		},
		{
			name: "not-yet-open takes precedence over expired",
			gift: domain.Gift{ScheduledOpenAt: ptr(future), ExpiresAt: ptr(past)},
			want: NotYetOpen,
		},
		{
			name: "expired takes precedence over already-opened",
			gift: domain.Gift{ExpiresAt: ptr(past), SingleOpen: true, OpenedAt: ptr(past)},
			want: Expired,
		},
	}
